...
from pyrit.models import AttackResult, Conversation, Message, MessagePiece, MessageScorable, Parameter
...
async def test_catalog_uses_registered_class_names_for_build_requests() -> None:
    registry = ScorerRegistry.get_registry_singleton()
    registry.register_class(SubStringScorer, name="custom-substring")
    service = ScorerService()

    catalog = await service.list_scorer_catalog_async()
    entry = next(item for item in catalog.items if item.scorer_type == "custom-substring")
    created = await service.create_scorer_async(
        request=CreateScorerRequest(type=entry.scorer_type, params={"substring": "WIN"})
    )
...
async def test_validate_scorer_accepts_inline_openai_target_extra_body_parameters_without_sends() -> None:
    ...
    with (
        patch.object(OpenAIChatTarget, "send_prompt_async", new_callable=AsyncMock) as mock_send_prompt,
        patch.object(service._memory, "add_scores_to_memory") as add_scores,
        patch.object(service._memory, "add_message_to_memory") as add_message,
    ):
    ...
    add_scores.assert_not_called()
    add_message.assert_not_called()
    assert service._registry.instances.get_names() == []
    assert service._target_registry.instances.get_names() == []
...
async def test_score_attack_result_uses_checked_anchor_snapshot_for_direct_scorer() -> None:
    ...
    # Upstream checks durable links at persistence time without re-acquiring scoring evidence.
    mock_get_message_pieces.assert_called_once()
    assert result.scores[0].message_piece_id == piece_id
    assert isinstance(result.scores[0].scorable, MessageScorable)
    assert result.scores[0].scorable.message_piece_ids == (piece_id,)
