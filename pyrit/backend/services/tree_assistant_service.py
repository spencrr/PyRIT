...
class _Session:
    ...
    task: asyncio.Task[AssistantRuntimeResult] | None = None
    fresh_turns: int = 0
    restored_requests: set[str] = field(default_factory=set)
    restored_proposals: set[str] = field(default_factory=set)
...
class TreeAssistantService:
    ...
    async def create_session_async(
        self, *, workspace_id: str, owner: str | None, history: list[TreeAssistantTurn] | None = None
    ) -> TreeAssistantSession:
        """
        Create a bounded owned session without making a model request.

        Returns:
            TreeAssistantSession: A fresh chat with optional client-reported historical turns.
        """
        restored = self._restore_transcript(workspace_id=workspace_id, history=history or [])
        ...
        try:
            ...
            if restored:
                runtime.restore_history(restored)
            public = TreeAssistantSession(
                session_id=secrets.token_urlsafe(32), workspace_id=workspace_id, model=runtime.model, turns=restored
            )
            self._sessions[public.session_id] = _Session(
                ...
                restored_requests={turn.request_id for turn in restored},
                restored_proposals={proposal.id for turn in restored for proposal in turn.proposals},
            )
    ...
    async def get_session_async(self, *, session_id: str, owner: str | None) -> TreeAssistantSession:
        """
        Get the latest 50 completed turns without exposing another owner's session.
    ...
    async def send_message_async(
        ...
        """
        session = await self._get_async(session_id=session_id, owner=owner)
        if request.request_id in session.restored_requests:
            raise TreeAssistantError(status=409, detail="Restored requests cannot be replayed; send a fresh message.")
        fingerprint = sha256(
        ...
        try:
            ...
            result = await asyncio.wait_for(session.task, timeout=self.TURN_TIMEOUT_SECONDS)
            turn = TreeAssistantTurn(
                request_id=request.request_id,
                message=request.message,
                reply=result.reply,
                proposals=[result.proposal] if result.proposal is not None else [],
                tool_calls=result.tool_calls,
                context_summary=result.context_summary,
                usage=result.usage,
            )
            session.public.turns.append(turn)
            # Only restored turns roll out before the live limit; fresh replies stay replayable.
            session.public.turns = session.public.turns[-self.MAX_TURNS :]
            session.fresh_turns += 1
    ...
    async def record_result_async(
        ...
        session = await self._get_async(session_id=session_id, owner=owner)
        self._require_idle(session)
        if proposal_id in session.restored_proposals:
            raise TreeAssistantError(status=409, detail="Restored history; re-plan before execution.")
        proposal = next(
    ...
    def _validate_message(self, *, session: _Session, request: TreeAssistantMessage) -> None:
        ...
        if session.fresh_turns >= self.MAX_TURNS:
            raise TreeAssistantError(
                status=429, detail="Assistant live turn limit reached; restart with context in a new session."
            )
        ...
        return [
            {
                ...
                **({"source": "restored_client_report"} if proposal.id in session.restored_proposals else {}),
            }
            for turn in session.public.turns
    ...
    @staticmethod
    def _restore_transcript(*, workspace_id: str, history: list[TreeAssistantTurn]) -> list[TreeAssistantTurn]:
        request = CreateTreeAssistantSession(workspace_id=workspace_id, history=history)
        restored = [turn.model_copy(deep=True) for turn in request.history]
        for turn in restored:
            if turn.context_summary:
                turn.context_summary.restored = True
                turn.context_summary.restoration_notice = (
                    "Client-reported historical context; not current server policy."
                )
            for proposal in turn.proposals:
                if proposal.status == "pending":
                    proposal.status = "failed"
                    proposal.result = TreeAssistantReceipt(
                        status="failed",
                        revision=proposal.base_revision,
                        detail="Restored history; re-plan before execution.",
                    )
                elif proposal.result:
                    proposal.result.detail = ("Client-reported restored receipt: " + proposal.result.detail)[:2_000]
        return restored
