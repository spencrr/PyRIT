...
"""Safe scorer catalog projection and recursive scorer construction helpers."""
...
class ScorerConfigurationManager:
    ...
    def _build_component_from_spec(
        ...
        *,
        ...
        resolved_by_name = {
            parameter.name: self._resolved_parameter(parameter=parameter, owner_cls=cls)
            for parameter in metadata.parameters
        }
        unknown = sorted(set(spec.params) - set(resolved_by_name))
        if unknown:
            raise ValueError(
                f"Unknown parameter(s) {unknown} for '{cls.__name__}'. Valid parameters: {sorted(resolved_by_name)}"
            )
        ...
        for name, value in spec.params.items():
            resolved_args[name] = self._resolve_parameter_value(
                owner_cls=cls,
                parameter=resolved_by_name[name],
                value=value,
                state=state,
                depth=depth,
            )
    ...
    def _namespace_callable_options(*, namespaces: Sequence[tuple[str, type]]) -> dict[str, Callable]:
        """Return a safe alias map for the public callables exposed by one or more namespaces."""
        options: dict[str, Callable] = {}
        ambiguous_simple_names: set[str] = set()
        for namespace_name, namespace in namespaces:
            for attr_name, value in vars(namespace).items():
                if attr_name.startswith("_") or not callable(value):
                    continue
                options[f"{namespace_name}.{attr_name}"] = value
                if attr_name in options and options[attr_name] is not value:
                    ambiguous_simple_names.add(attr_name)
                else:
                    options[attr_name] = value
                callable_name = getattr(value, "__name__", None)
                if callable_name is not None:
                    options[callable_name] = value
