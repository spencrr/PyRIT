...
class ScorerService:
    ...
    async def list_scorer_catalog_async(self) -> ScorerCatalogResponse:
        ...
        """
        return await asyncio.to_thread(self._build_catalog)
    ...
    def _build_catalog(self) -> ScorerCatalogResponse:
        """
        Project the registry catalog, including trusted annotation and adapter discovery.

        Returns:
            ScorerCatalogResponse: API-safe metadata keyed by the registry's build names.
        """
        metadata_items = self._registry.get_all_registered_class_metadata()
        items = []
        for metadata in metadata_items:
            owner_cls = self._registry.get_class(metadata.registry_name)
            items.append(
                ScorerCatalogEntry(
                    scorer_type=metadata.registry_name,
                    score_type=self._score_type_for_class_name(class_name=metadata.registry_name),
