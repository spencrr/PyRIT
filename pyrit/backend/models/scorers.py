...
class ScorerParameter(Parameter):
    ...
    accepted_types: list[str] = Field(
        ...
        description="Compatible registry build names for inline specs; registered instance aliases are in choices.",
    )
...
class ScorerCatalogEntry(BaseModel):
    ...
    scorer_type: str = Field(
        ..., description="Registry build name (e.g., 'SubStringScorer' or a registered class alias)"
    )
...
class CreateScorerRequest(BaseModel):
    ...
    type: Annotated[str, StringConstraints(strip_whitespace=True)] = Field(
        ..., min_length=1, max_length=200, description="Scorer type (e.g., 'SubStringScorer')"
    )
...
class ScoreAttackRequest(BaseModel):
    ...
    attack_result_id: Annotated[str, StringConstraints(strip_whitespace=True)] = Field(
        ..., min_length=1, max_length=255, description="Attack result ID to score"
    )
    conversation_id: Annotated[str, StringConstraints(strip_whitespace=True)] = Field(
        ..., min_length=1, max_length=255, description="Conversation ID to score"
    )
    expected_scorer_hash: Annotated[str, StringConstraints(strip_whitespace=True)] = Field(
        ...,
        min_length=64,
        max_length=64,
