            ...
            try:
                if request.method == "POST":
                    ...
                    async for chunk in request.stream():
                        body.extend(chunk)
                        if len(body) > 600_000:
                            raise TreeAssistantError(status=413, detail="Assistant request exceeds 600000 bytes.")
...
async def create_session_async(*, request: Request, body: CreateTreeAssistantSession) -> TreeAssistantSession:
    """
    Create an ephemeral assistant session without calling the model.

    Returns:
        TreeAssistantSession: The empty session.
    """
    return await get_tree_assistant_service(request).create_session_async(
        workspace_id=body.workspace_id, owner=_owner(request)
    )
...
async def get_session_async(*, request: Request, session_id: str) -> TreeAssistantSession:
    """
    Recover completed turns, including their current proposal receipts.

    Returns:
        TreeAssistantSession: Completed turns in this session.
    """
