...
function matchesAcceptedTypes(candidates: string[], acceptedTypes?: string[]): boolean {
  if ((acceptedTypes?.length ?? 0) === 0) {
    return true
  }
  return candidates.some((candidate: string) => acceptedTypes?.includes(candidate) === true)
}
...
function compatibleScorerCatalog(
  ...
  return catalog.filter((entry: ScorerCatalogEntry) => matchesAcceptedTypes([entry.scorer_type], parameter.accepted_types))
}
...
function compatibleInstances(
  ...
  return instances.filter((instance: ScorerInstance) => matchesAcceptedTypes([instance.scorer_type], parameter.accepted_types))
}
...
function compatibleTargets(
  ...
  return targets.filter((target: TargetInstance) =>
    matchesAcceptedTypes([target.identifier.class_name, target.target_registry_name], parameter.accepted_types))
}
