...
export default function TreeScoreMeter({ node, settings }: TreeScoreMeterProps) {
  ...
  const runs = primary ? node.scoreRuns?.filter((result) =>
    result.scorerId === primary.scorer_id && result.scorerHash === primary.identifier_hash) : undefined
  ...
  const label = !primary ? 'No scorer'
    : !run ? 'Unscored' : run.status === 'error' ? 'Scoring error'
