...
describe('treeStorage', () => {
  ...
  it('should reject malformed optional attempt fields without rewriting storage', async () => {
    const draft = applyTreeCommand(createTreeWorkspace(CONFIGURATION), { type: 'add', parentId: null, prompt: 'hello' })
    const saved = await saveTreeWorkspace(draft)
    const key = `pyrit:conversation-tree:v1:${saved.id}`
    const malformed = JSON.stringify({ ...saved, nodes: [{ ...saved.nodes[0], attemptId: null, attackResultId: 42 }] })
    localStorage.setItem(key, malformed)
    expect(() => loadTreeWorkspace(saved.id)).toThrow('malformed')
    await expect(saveTreeWorkspace(saved)).rejects.toThrow('malformed')
    expect(localStorage.getItem(key)).toBe(malformed)
  })

  it('should archive exactly the shared current attempt without presentation state', async () => {
    const draft = applyTreeCommand(createTreeWorkspace(CONFIGURATION), { type: 'add', parentId: null, prompt: 'hello' })
    const observed = await saveTreeWorkspace({
      ...draft, nodes: [{ ...draft.nodes[0], status: 'error', error: 'Failed before sending' }],
    })
    const retried = await saveTreeWorkspace(applyTreeCommand(observed, { type: 'retry', nodeId: observed.nodes[0].id, scope: 'node' }))
    expect(retried.nodes[0].attempts?.[0]).toMatchObject({
      attemptId: observed.nodes[0].attemptId, status: 'error', error: 'Failed before sending', prompt: 'hello', converters: [],
    })
    expect(retried.nodes[0].attempts?.[0]).not.toHaveProperty('position')
    const archived = retried.nodes[0].attempts?.[0]
    if (!archived) throw new Error('Expected an archived attempt')
    await expect(saveTreeWorkspace({
      ...retried, nodes: [{ ...retried.nodes[0], attempts: [{ ...archived, error: 'Rewritten' }] }],
