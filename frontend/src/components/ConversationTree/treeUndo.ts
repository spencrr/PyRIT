import type { TreeUndoEditableField, TreeUndoEntry, TreeUndoNode, TreeWorkspace } from '@/types'

import { applyTreeCommand, getCurrentAttemptId } from './treeModel'

export type { TreeUndoEntry } from '@/types'

const FIELDS: TreeUndoEditableField[] = ['prompt', 'converters', 'pruned', 'kept', 'position', 'size']

export function captureTreeUndo(before: TreeWorkspace, after: TreeWorkspace): TreeUndoEntry {
  const nodes: TreeUndoNode[] = []
