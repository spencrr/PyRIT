...
describe('TreeScoringDialog', () => {
  ...
  it('refreshes backend registered choices after creating a scorer', async () => {
    const user = userEvent.setup()
    const inverter: ScorerCatalogEntry = {
      scorer_type: 'TrueFalseInverterScorer',
      score_type: 'true_false',
      is_llm_based: false,
      parameters: [{
        name: 'scorer',
    ...
    const refreshed = {
      ...inverter,
      parameters: [{ ...inverter.parameters[0], choices: [REGISTERED_SUBSTRING.scorer_id] }],
    }
    jest.mocked(scorersApi.listCatalog)
      .mockResolvedValueOnce({ items: [SUBSTRING_ENTRY, inverter] })
      .mockResolvedValue({ items: [SUBSTRING_ENTRY, refreshed] })
    jest.mocked(scorersApi.listScorers).mockResolvedValueOnce({ items: [] })
    renderDialog()
    await openBuilder(user, 'SubStringScorer')
    await user.type(screen.getByRole('textbox', { name: /substring/ }), ' marker ')
    await user.click(screen.getByRole('button', { name: 'Create and select scorer' }))
    await screen.findByText('SubStringScorer is ready to use.')
    await user.selectOptions(screen.getByLabelText('Scorer type'), 'TrueFalseInverterScorer')

    const configuration = screen.getByRole('region', { name: 'TrueFalseInverterScorer configuration' })
    expect(within(within(configuration).getByRole('combobox')).getByRole('option', {
      name: /substring-test/,
    })).toBeInTheDocument()
    expect(scorersApi.listCatalog).toHaveBeenCalledTimes(2)
    expect(scorersApi.createScorer).toHaveBeenCalledWith({
      type: 'SubStringScorer', params: { substring: ' marker ' },
    })
  })

  it('keeps a successfully created scorer if refreshing its catalog fails', async () => {
    const user = userEvent.setup()
    jest.mocked(scorersApi.listCatalog)
      .mockResolvedValueOnce({ items: [SUBSTRING_ENTRY] })
    ...
    expect(await screen.findByText(/SubStringScorer is ready to use. Reopen scoring settings/)).toBeVisible()
    expect(screen.getByRole('region', { name: 'SubStringScorer' })).toBeInTheDocument()
