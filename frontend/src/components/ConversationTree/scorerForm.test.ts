...
describe('buildScorerParameters', () => {
  it.each([' WIN \n', ' '])('preserves free-text whitespace for %p', (substring: string) => {
    expect(buildScorerParameters(SUBSTRING.parameters, { substring })).toEqual({
      ok: true, parameters: { substring },
    })
  })

  it('preserves nested regex and prompt payloads and only trims reference identifiers', () => {
    const entry: ScorerCatalogEntry = {
      ...SUBSTRING,
      parameters: [
        ...SUBSTRING.parameters,
        { name: 'patterns', type_name: 'dict[str,str]', required: true, input_kind: 'json' },
      ],
    }
    expect(buildScorerParameters(THRESHOLD.parameters, {
      threshold: '0.5',
      nested: createInlineReferenceValue('SubStringScorer', {
        substring: ' WIN \n',
        patterns: { format: 'json', value: '{" regex ": " ^WIN$ \\n"}' },
      }),
      fallbacks: [createRegisteredReferenceValue(' registered ')],
    }, { catalog: [entry] })).toEqual({
      ok: true,
      parameters: {
        threshold: 0.5,
        nested: { type: 'SubStringScorer', params: { substring: ' WIN \n', patterns: { ' regex ': ' ^WIN$ \n' } } },
  ...
  it('builds nested scorer payloads for single and list references', () => {
    const parameters = THRESHOLD.parameters
  ...
  it('omits unset optional singular registered scorer references', () => {
    const parameters = VIDEO_TRUE_FALSE.parameters
