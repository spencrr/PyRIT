...
const SYNTHETIC_JUDGE_PARAMETER: ScorerParameter = {
  name: 'chat_target',
  type_name: 'PromptChatTarget',
  required: true,
  input_kind: 'field',
  reference_kind: 'target',
  description: 'Choose the registered model that will evaluate responses.',
}
...
function buildReferenceValue(
  ...
  const nestedParameters = buildScorerParametersInternal(
    getScorerBuilderParameters(nestedEntry),
    reference.values,
...
function buildScorerParametersInternal(
  ...
  const simpleResult = buildParametersFromForm(simpleParameters, simpleValues)
  if (!simpleResult.ok) {
    return simpleResult
  }
...
export function getScorerBuilderParameters(entry: ScorerCatalogEntry): ScorerParameter[] {
  const hasExplicitJudge = entry.parameters.some((parameter: ScorerParameter) => parameter.reference_kind === 'target' || parameter.name === 'chat_target')
  return entry.is_llm_based && !hasExplicitJudge
    ? [...entry.parameters, SYNTHETIC_JUDGE_PARAMETER]
    : entry.parameters
}
...
export function getInitialScorerFormValues(
  ...
  const parameters = getScorerBuilderParameters(entry)
...
export function findUnsupportedScorerConfiguration(
  ...
  return findUnsupportedConfigurationInternal(getScorerBuilderParameters(entry), values, catalog, 1)
}
