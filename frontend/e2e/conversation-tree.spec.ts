...
async function workspaceAction(page: Page, name: string): Promise<void> {
...
test.describe('Conversation tree @seeded', () => {
  test('edits and reloads a 300-node workspace without sending requests', async ({ page, request }) => {
    ...
    await dialog.getByLabel('Strategy plan JSON').fill(JSON.stringify({
      ...
      steps: Array.from({ length: 300 }, (_, index) => ({
        id: `step-${index}`,
        parentId: index === 0 ? null : `step-${Math.floor((index - 1) / 3)}`,
        prompt: `Capacity prompt ${index}`,
        converters: [],
      })),
    }))
    const sends: string[] = []
    ...
    const selected = await page.evaluate(() => {
      const key = Object.keys(localStorage).find((name) => name.startsWith('pyrit:conversation-tree:v1:')
        && JSON.parse(localStorage.getItem(name) ?? '{}').nodes?.length === 300)
      if (!key) throw new Error('Capacity workspace was not saved')
      return key
    })
    await page.getByRole('complementary', { name: 'Tree outline' })
      .getByRole('button', { name: 'Capacity prompt 299 (draft)', exact: true }).click()
    await page.getByLabel('Prompt', { exact: true }).fill('Edited leaf at capacity')
    await page.getByRole('button', { name: 'Save draft', exact: true }).click()
    await expect(page.getByLabel('Prompt', { exact: true })).toHaveValue('Edited leaf at capacity')
    const stored = await page.evaluate((key) => JSON.parse(localStorage.getItem(key) ?? 'null'), selected)
    expect(stored.nodes).toHaveLength(300)
    expect(stored.nodes[299].prompt).toBe('Edited leaf at capacity')
    await page.reload()
  ...
  test('creates child variants and original-inclusive sample groups, retains edits and persists forks', async ({ page, request }) => {
    ...
    await workspaceAction(page, 'Show pruned')
  ...
  test('keeps every branch on canvas through outline selection, drag, auto layout and reload', async ({ page, request }) => {
    ...
    await workspaceAction(page, 'Auto layout')
  ...
  test('constructs nested scorers and YAML SeedPrompts with side-effect-free preflight', async ({ page, request }) => {
    await createDraftTree(page, request)
    const tree = await savedTree(page)
    await workspaceAction(page, 'Scoring')
  ...
  test('starts a new workspace outside the previous focused group and edit history', async ({ page, request }) => {
    ...
    await page.getByRole('button', { name: 'Workspace options' }).click()
    await expect(page.getByRole('menuitem', { name: 'Undo edit', exact: true })).toBeDisabled()
